...
export class TreasureMap {
  ...
  updateCoordinateMap() {
    this.coordinateToTreasureMap = Object.values(this.map).reduce(
      (coordinateToTreasureMap, treasureInfo) => {
  ...
  moveTreasure(treasure: Treasure, newCoordinate: Coordinate) {
    if (this.map[treasure.id]) {
      this.map[treasure.id].coordinate = newCoordinate;
      this.updateCoordinateMap();
    }
  }
