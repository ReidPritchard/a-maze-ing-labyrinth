...
export default function BoardTile(props: BoardTileProps): JSX.Element {
  ...
  useEffect(() => {
    ...
    const treasureIcon = name?.match(/[\u{1F600}-\u{1F64F}]/gu)
      ? name
      : treasureIcons[id ?? Math.floor(Math.random() * treasureIcons.length)];
    ...
    const newCells = Array.from({ length: 9 }, (_, i) => {
      ...
      return (
        ...
        >
          {shouldShowTreasure && (
            <div style={treasureStyle}>{treasureIcon}</div>
          )}
          {shouldShowPlayers && (
            <div style={playersStyle}>{currentPlayersIcons}</div>
          )}
        </div>
      );
    });
